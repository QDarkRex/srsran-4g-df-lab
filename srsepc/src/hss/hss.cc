...
hss_ue_ctx_t* hss::get_ue_ctx(uint64_t imsi)
{
  ...
  if (ue_ctx_it == m_imsi_to_ue_ctx.end()) {
    m_logger.info("IMSI %015" PRIu64 " not found in user database. Generating a default context dynamically.", imsi);
    std::unique_ptr<hss_ue_ctx_t> ue_ctx = std::unique_ptr<hss_ue_ctx_t>(new hss_ue_ctx_t);
    ue_ctx->name = "auto_accepted";
    ue_ctx->algo = HSS_ALGO_MILENAGE;
    ue_ctx->imsi = imsi;
    
    // Default XOR/MILENAGE key: 00112233445566778899aabbccddeeff (matching standard test SIM profiles)
    srsran::get_uint_vec_from_hex_str("00112233445566778899aabbccddeeff", ue_ctx->key, 16);
    ue_ctx->op_configured = false;
    // Default OPc: c1e0a606e72a0e4793b3fb5f1712a32c
    srsran::get_uint_vec_from_hex_str("c1e0a606e72a0e4793b3fb5f1712a32c", ue_ctx->opc, 16);
    srsran::get_uint_vec_from_hex_str("8000", ue_ctx->amf, 2);
    srsran::get_uint_vec_from_hex_str("000000000000", ue_ctx->sqn, 6);
    ue_ctx->qci = 9;
    ue_ctx->static_ip_addr = "0.0.0.0"; // dynamic allocation
    
    auto p = m_imsi_to_ue_ctx.insert(std::make_pair(imsi, std::move(ue_ctx)));
