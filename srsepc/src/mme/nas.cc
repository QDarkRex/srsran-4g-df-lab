...
static void write_csi_imsi_mapping(uint32_t enb_ue_s1ap_id, uint64_t imsi)
{
  FILE* fp_s1ap_imsi = fopen("/tmp/s1ap_imsi.csv", "a");
  if (fp_s1ap_imsi != NULL) {
    fprintf(fp_s1ap_imsi, "%u,%015" PRIu64 "\n", enb_ue_s1ap_id, imsi);
    fclose(fp_s1ap_imsi);
  }

  FILE* fp_rnti_s1ap = fopen("/tmp/rnti_s1ap.csv", "r");
  if (fp_rnti_s1ap == NULL) {
    return;
  }

  uint16_t latest_rnti = 0;
  bool     found_rnti  = false;
  char     line[256];
  while (fgets(line, sizeof(line), fp_rnti_s1ap) != NULL) {
    unsigned int rnti = 0;
    unsigned int s1ap = 0;
    if (sscanf(line, "%u,%u", &rnti, &s1ap) == 2 && s1ap == enb_ue_s1ap_id) {
      latest_rnti = (uint16_t)rnti;
      found_rnti  = true;
    }
  }
  fclose(fp_rnti_s1ap);
                                ...
                                const nas_if_t&         itf)
{
  ...
  if (attach_req.eps_mobile_id.type_of_id == LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI) {
    ...
    write_csi_imsi_mapping(enb_ue_s1ap_id, imsi);
                                                ...
                                                const nas_if_t&                                       itf)
{
  ...
  // Write mapping of enb_ue_s1ap_id/RNTI -> imsi to CSI mapping files.
  write_csi_imsi_mapping(nas_ctx->m_ecm_ctx.enb_ue_s1ap_id, nas_ctx->m_emm_ctx.imsi);
  
  // Console logging when Auth Request is triggered
  srsran::console("[srsRAN-AuthDebug] NAS Authentication Request triggered for IMSI: %015" PRIu64 "\n", nas_ctx->m_emm_ctx.imsi);

  // Staging: Bypass packing and sending of Authentication Request (ignore Auth)
  /*
  ...
  */
  srsran::console("[srsRAN] Authentication Request bypassed: ignoring/silently dropping auth responses to UE.\n");
  return true;
}
                                              ...
                                              const nas_if_t&                                       itf)
{
  ...
  if (msg_valid == true && emm_ctx->state == EMM_STATE_DEREGISTERED) {
    ...
    // Console logging when Auth Request is triggered
    srsran::console("[srsRAN-AuthDebug] NAS Authentication Request triggered for GUTI (IMSI: %015" PRIu64 ")\n", emm_ctx->imsi);
    ...
    */
    srsran::console("[srsRAN] GUTI NAS integrity failed - Authentication Request bypassed.\n");
    return true;
  }
}
...
bool nas::handle_identity_response(srsran::byte_buffer_t* nas_rx)
{
  ...
  // Write mapping of enb_ue_s1ap_id/RNTI -> imsi to CSI mapping files.
  write_csi_imsi_mapping(m_ecm_ctx.enb_ue_s1ap_id, m_emm_ctx.imsi);

  // Console logging when Auth Request is triggered
  srsran::console("[srsRAN-AuthDebug] NAS Authentication Request triggered for Identity Response (IMSI: %015" PRIu64 ")\n", imsi);
  ...
  srsran::console("Downlink NAS: Sent Authentication Request\n");
  */
  srsran::console("[srsRAN] Identity Response received - Authentication Request bypassed.\n");
  return true;
}
...
bool nas::handle_authentication_failure(srsran::byte_buffer_t* nas_rx)
{
  ...
  switch (auth_fail.emm_cause) {
    ...
    case 21:
      ...
      // Console logging when Auth Request is triggered
      srsran::console("[srsRAN-AuthDebug] NAS Authentication Request triggered for Auth Failure (IMSI: %015" PRIu64 ")\n", m_emm_ctx.imsi);
      ...
      */
      srsran::console("[srsRAN] Authentication Failure processed - Authentication Request bypassed.\n");
      break;
  }
  return true;
}
