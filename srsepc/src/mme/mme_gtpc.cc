...
bool mme_gtpc::init()
{
  ...
  // Clear /tmp/s1ap_imsi.csv and /tmp/active_imsi
  FILE* fp_clear = fopen("/tmp/s1ap_imsi.csv", "w");
  if (fp_clear != NULL) {
    fclose(fp_clear);
  }
  fp_clear = fopen("/tmp/active_imsi", "w");
  if (fp_clear != NULL) {
    fclose(fp_clear);
  }
...
bool mme_gtpc::handle_create_session_response(srsran::gtpc_pdu* cs_resp_pdu)
{
  ...
  // Write active IMSI to /tmp/active_imsi for the channel estimator
  FILE* fp_imsi = fopen("/tmp/active_imsi", "w");
  if (fp_imsi != NULL) {
    fprintf(fp_imsi, "%015" PRIu64 "\n", emm_ctx->imsi);
    fclose(fp_imsi);
  }

  // Write mapping of enb_ue_s1ap_id -> imsi to /tmp/s1ap_imsi.csv
  FILE* fp_s1ap_imsi = fopen("/tmp/s1ap_imsi.csv", "a");
  if (fp_s1ap_imsi != NULL) {
    fprintf(fp_s1ap_imsi, "%u,%015" PRIu64 "\n", ecm_ctx->enb_ue_s1ap_id, emm_ctx->imsi);
    fclose(fp_s1ap_imsi);
  }

  // Trigger rapid ping command for this UE in background
  char ping_cmd[128];
  snprintf(ping_cmd, sizeof(ping_cmd), "ping %s -i 0.2 &", inet_ntoa(emm_ctx->ue_ip));
  srsran::console("Starting rapid ping command in background: %s\n", ping_cmd);
  int sys_res = system(ping_cmd);
  (void)sys_res;
