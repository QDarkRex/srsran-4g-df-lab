...
/* Modified Init to accept antenna_idx */
int srsran_enb_ul_init(srsran_enb_ul_t* q, cf_t* in_buffer, uint32_t max_prb, uint32_t antenna_idx)
{
  ...
  if (q != NULL) {
    ...
    // Store the antenna index for this instance
    q->antenna_idx = antenna_idx; 
...
static int get_pucch(srsran_enb_ul_t* q, srsran_ul_sf_cfg_t* ul_sf, srsran_pucch_cfg_t* cfg, srsran_pucch_res_t* res)
{
  ...
  for (int i = 0; i < nof_resources && ret == SRSRAN_SUCCESS; i++) {
    ...
    /* DF MODIFICATION: Set antenna index for PUCCH estimation */
    q->chest_res.antenna_idx = q->antenna_idx;
...
{
  /* DF MODIFICATION: Set antenna index for PUSCH estimation */
  q->chest_res.antenna_idx = q->antenna_idx;
