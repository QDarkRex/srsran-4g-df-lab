...
int s1ap::init(const s1ap_args_t& args_, rrc_interface_s1ap* rrc_)
{
  ...
  // Clear CSI identity mapping files on startup.
  FILE* fp_clear = fopen("/tmp/rnti_s1ap.csv", "w");
  if (fp_clear != NULL) {
    fclose(fp_clear);
  }
  fp_clear = fopen("/tmp/rnti_imsi.csv", "w");
  if (fp_clear != NULL) {
    fclose(fp_clear);
  }
...
static void write_csi_imsi_mapping(uint32_t enb_ue_s1ap_id, uint16_t rnti, uint64_t imsi)
{
  FILE* fp_s1ap_imsi = fopen("/tmp/s1ap_imsi.csv", "a");
  if (fp_s1ap_imsi != nullptr) {
    fprintf(fp_s1ap_imsi, "%u,%015" PRIu64 "\n", enb_ue_s1ap_id, imsi);
    fclose(fp_s1ap_imsi);
  }

  FILE* fp_rnti_imsi = fopen("/tmp/rnti_imsi.csv", "a");
  if (fp_rnti_imsi != nullptr) {
    fprintf(fp_rnti_imsi, "%u,%015" PRIu64 "\n", rnti, imsi);
    fclose(fp_rnti_imsi);
  }
}

static void log_csi_imsi_debug(uint32_t enb_ue_s1ap_id,
                               uint16_t rnti,
                               const char* reason,
                               const srsran::unique_byte_buffer_t& pdu)
{
  FILE* fp = fopen("/tmp/csi_imsi_debug.log", "a");
  if (fp == nullptr) {
    return;
  }
  ...
  if (pdu && pdu->N_bytes <= sizeof(nas_msg.msg)) {
    nas_msg.N_bytes = pdu->N_bytes;
    memcpy(nas_msg.msg, pdu->msg, pdu->N_bytes);
    (void)liblte_mme_parse_msg_header(&nas_msg, &pd, &msg_type);
  }

  fprintf(fp,
          "rnti=%u enb_ue_s1ap_id=%u reason=%s bytes=%u pd=0x%02x msg_type=0x%02x hex=",
          rnti,
          enb_ue_s1ap_id,
          reason,
          pdu ? pdu->N_bytes : 0,
          ...
          msg_type);
  if (pdu) {
    uint32_t dump_len = SRSRAN_MIN(pdu->N_bytes, 160U);
    for (uint32_t i = 0; i < dump_len; i++) {
      fprintf(fp, "%02x", pdu->msg[i]);
    }
  }
  fprintf(fp, "\n");
  fclose(fp);
...
static bool copy_nas_view(const srsran::unique_byte_buffer_t& pdu, uint32_t offset, LIBLTE_BYTE_MSG_STRUCT* nas_msg)
{
  if (!pdu || nas_msg == nullptr || offset >= pdu->N_bytes || pdu->N_bytes - offset > sizeof(nas_msg->msg)) {
    return false;
  }

  memset(nas_msg, 0, sizeof(*nas_msg));
  nas_msg->N_bytes = pdu->N_bytes - offset;
  memcpy(nas_msg->msg, &pdu->msg[offset], nas_msg->N_bytes);
  return true;
}

static bool try_extract_and_write_imsi_from_nas_msg(uint32_t enb_ue_s1ap_id,
                                                    uint16_t rnti,
                                                    LIBLTE_BYTE_MSG_STRUCT* nas_msg)
{
  uint8_t pd       = 0;
  uint8_t msg_type = 0;
  if (liblte_mme_parse_msg_header(nas_msg, &pd, &msg_type) != LIBLTE_SUCCESS) {
    return false;
  }

  if (msg_type == LIBLTE_MME_MSG_TYPE_ATTACH_REQUEST) {
    LIBLTE_MME_ATTACH_REQUEST_MSG_STRUCT attach_req = {};
    if (liblte_mme_unpack_attach_request_msg(nas_msg, &attach_req) == LIBLTE_SUCCESS &&
        attach_req.eps_mobile_id.type_of_id == LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI) {
      uint64_t imsi = 0;
      for (int i = 0; i <= 14; i++) {
        imsi = imsi * 10 + attach_req.eps_mobile_id.imsi[i];
      }
      write_csi_imsi_mapping(enb_ue_s1ap_id, rnti, imsi);
      return true;
    }
  }
  ...
  if (msg_type == LIBLTE_MME_MSG_TYPE_IDENTITY_RESPONSE) {
    ...
    if (liblte_mme_unpack_identity_response_msg(nas_msg, &id_resp) == LIBLTE_SUCCESS &&
...
static bool try_extract_and_write_imsi(uint32_t enb_ue_s1ap_id, uint16_t rnti, const srsran::unique_byte_buffer_t& pdu)
{
  LIBLTE_BYTE_MSG_STRUCT nas_msg = {};
  if (copy_nas_view(pdu, 0, &nas_msg) && try_extract_and_write_imsi_from_nas_msg(enb_ue_s1ap_id, rnti, &nas_msg)) {
    return true;
  }

  // Be tolerant of captures/forwarders that include one leading wrapper byte before the NAS PDU.
  if (copy_nas_view(pdu, 1, &nas_msg) && try_extract_and_write_imsi_from_nas_msg(enb_ue_s1ap_id, rnti, &nas_msg)) {
    return true;
  }

  log_csi_imsi_debug(enb_ue_s1ap_id, rnti, "no_imsi_found", pdu);
  return false;
}
                      ...
                      srsran::unique_byte_buffer_t          pdu)
{
  ...
  try_extract_and_write_imsi(u->ctxt.enb_ue_s1ap_id, rnti, pdu);
...
void s1ap::user_mod(uint16_t old_rnti, uint16_t new_rnti)
{
  ...
  ue* u = users.find_ue_rnti(old_rnti);
  if (u != nullptr) {
    u->ctxt.rnti = new_rnti;
    // Write new RNTI to enb_ue_s1ap_id mapping to /tmp/rnti_s1ap.csv
    FILE* fp_rnti = fopen("/tmp/rnti_s1ap.csv", "a");
    if (fp_rnti != NULL) {
      fprintf(fp_rnti, "%u,%u\n", new_rnti, u->ctxt.enb_ue_s1ap_id);
      fclose(fp_rnti);
    }
  }
}
...
s1ap::ue* s1ap::user_list::add_user(std::unique_ptr<s1ap::ue> user)
{
  ...
  if (p.second) {
    // Write RNTI to enb_ue_s1ap_id mapping to /tmp/rnti_s1ap.csv
    FILE* fp_rnti = fopen("/tmp/rnti_s1ap.csv", "a");
    if (fp_rnti != NULL) {
      fprintf(fp_rnti, "%u,%u\n", p.first->second->ctxt.rnti, p.first->second->ctxt.enb_ue_s1ap_id);
      fclose(fp_rnti);
    }
