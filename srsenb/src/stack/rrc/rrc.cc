                  ...
                  rrc_nr_interface_rrc*  rrc_nr_)
{
  ...
  int res_rm = std::system("rm -f /tmp/multipath_*.csv /tmp/csi_capture.bin /tmp/rnti_s1ap.csv /tmp/s1ap_imsi.csv /tmp/active_imsi /tmp/imsi");
  (void)res_rm;
...
{
  tti_point t = tti_point(tti);
  uint32_t sf = t.sf_idx();
  if (sf == 0 || sf == 4 || sf == 5 || sf == 9) {
    size_t bytes = pending_paging->pending_pcch_bytes(t);
    if (bytes > 0) {
      *payload_len = bytes;
    } else {
      *payload_len = 4; // dummy empty paging message size
    }
    return true;
  }
  return false;
...
void rrc::read_pdu_pcch(uint32_t tti_tx_dl, uint8_t* payload, uint32_t buffer_size)
{
  ...
  if (not pending_paging->read_pdu_pcch(tti_point(tti_tx_dl), read_func)) {
    // Generate dummy empty paging message
    pcch_msg_s pcch_msg;
    pcch_msg.msg.set_c1().paging();
    pcch_msg.msg.c1().paging().paging_record_list_present = false;
    pcch_msg.msg.c1().paging().sys_info_mod_present = true;
    srsran::unique_byte_buffer_t dummy_pdu = srsran::make_byte_buffer();
    if (dummy_pdu != nullptr) {
      asn1::bit_ref bref(dummy_pdu->msg, dummy_pdu->get_tailroom());
      if (pcch_msg.pack(bref) != asn1::SRSASN_ERROR_ENCODE_FAIL) {
        dummy_pdu->N_bytes = (uint32_t)bref.distance_bytes();
        if (dummy_pdu->size() <= buffer_size) {
          std::copy(dummy_pdu->msg, dummy_pdu->msg + dummy_pdu->size(), payload);
        }
      }
    }
  }
...
{
  // Force modification period to n2 (2 * paging cycle) to allow faster SIB1 updates (640ms)
  cfg.sibs[1].sib2().rr_cfg_common.bcch_cfg.mod_period_coeff.value = asn1::rrc::bcch_cfg_s::mod_period_coeff_opts::n2;
...
{
  // ==========================================================================
  // STALLING MODIFICATION: 5-SECOND IDENTITY SHIFT WITH TOTAL CONTEXT FLUSH
  // ==========================================================================
  static uint32_t stalling_ms_counter = 0;
  stalling_ms_counter++;

  // 10000 TTIs = 10000 ms = 10.00 seconds
  if (stalling_ms_counter >= 1000) {
    stalling_ms_counter = 0; 

    // 1. Collect active RNTIs using standard C++11 auto reference loop matching keys safely
    std::vector<uint16_t> active_rntis;
    for (auto const& pair : users) {
      active_rntis.push_back(pair.first);
    }
    
    // Explicitly delete and flush the collected user contexts
    for (size_t i = 0; i < active_rntis.size(); ++i) {
      uint16_t rnti = active_rntis[i];
      // rem_user(rnti); 
    }

    // 2. Rotate the System Info Value Tag (0-31)
    cfg.sib1.sys_info_value_tag = (cfg.sib1.sys_info_value_tag + 1) % 32;

    // 3. Extract current identities from the first cell config directly (plain C++ types)

    uint32_t raw_tac = cfg.cell_list[0].tac;
    uint32_t raw_cell_id = cfg.cell_list[0].cell_id;

    // Compute new values
    uint32_t new_tac = (raw_tac + 9) % 65535;
    uint32_t new_cell_id = (raw_cell_id + 144) % 268435456;

    // Apply updates to the master config context
    cfg.sib1.cell_access_related_info.tac.from_number(new_tac);
    cfg.sib1.cell_access_related_info.cell_id.from_number(new_cell_id);

    for (uint32_t i = 0; i < cfg.cell_list.size(); ++i) {
      cfg.cell_list[i].tac = new_tac;
      cfg.cell_list[i].cell_id = new_cell_id;
    }



    // ========================================================================
    // CRITICAL FIX 1: UPDATE THE CACHED CELL OBJECTS DIRECTLY BEFORE RE-PACK
    // ========================================================================
    if (cell_common_list) {
      for (uint32_t i = 0; i < cell_common_list->size(); ++i) {
        enb_cell_common* cell = const_cast<enb_cell_common*>(cell_common_list->get_cell_id(i));
        if (cell != nullptr) {
          // Mutate structural dependencies inside the target structures
          cell->sib1.sys_info_value_tag = cfg.sib1.sys_info_value_tag;
          cell->sib1.cell_access_related_info.tac.from_number(new_tac);
          cell->sib1.cell_access_related_info.cell_id.from_number(new_cell_id);
          
          // Clear old pre-compiled byte vectors completely
          cell->sib_buffer.clear();
        }
      }
    }

    // Force RRC to rebuild raw ASN.1 payloads using updated cell parameters
    if (generate_sibs() != SRSRAN_SUCCESS) {
      logger.error("Failed to compile new SIB ASN.1 payloads.");
    }

    // ========================================================================
    // CRITICAL FIX 2: PROPAGATE DYNAMIC PAYLOAD CHANGES DOWN TO THE MAC LAYER
    // ========================================================================
    std::vector<sched_interface::cell_cfg_t> sched_cfg;
    sched_cfg.resize(cfg.cell_list.size());
    ...
    for (uint32_t ccidx = 0; ccidx < cfg.cell_list.size(); ++ccidx) {
      ...
      for (uint32_t i = 0; i < nof_si_messages; i++) {
        auto& buffer = cell_common_list->get_cc_idx(ccidx)->sib_buffer.at(i);
        if (buffer.is_segmented()) {
          for (unsigned i_seg = 0, nof_segs = buffer.get_nof_segments(); i_seg != nof_segs; ++i_seg) {
            item.sibs[i].add_segment(cell_common_list->get_cc_idx(ccidx)->sib_buffer.at(i).get_segment(i_seg)->N_bytes);
          }
        } else {
          item.sibs[i].add_segment(cell_common_list->get_cc_idx(ccidx)->sib_buffer.at(i).get()->N_bytes);
        }
        if (i == 0) {
          item.sibs[i].set_period_rf(8); 
        } else {
          item.sibs[i].set_period_rf(cfg.sib1.sched_info_list[i - 1].si_periodicity.to_number());
        }
      }
      ...
      item.prach_nof_preambles = cfg.sibs[1].sib2().rr_cfg_common.rach_cfg_common.preamb_info.nof_ra_preambs.to_number();
      item.si_window_ms        = cfg.sib1.si_win_len.to_number();
      item.prach_rar_window    = cfg.sibs[1].sib2().rr_cfg_common.rach_cfg_common.ra_supervision_info.ra_resp_win_size.to_number();
      item.prach_freq_offset    = cfg.sibs[1].sib2().rr_cfg_common.prach_cfg.prach_cfg_info.prach_freq_offset;
      item.maxharq_msg3tx       = cfg.sibs[1].sib2().rr_cfg_common.rach_cfg_common.max_harq_msg3_tx;
      item.enable_64qam         = cfg.sibs[1].sib2().rr_cfg_common.pusch_cfg_common.pusch_cfg_basic.enable64_qam;
      item.target_pucch_ul_sinr = cfg.cell_list[ccidx].target_pucch_sinr_db;
    ...
    // Direct interface push to flush MAC schedulers active transport queues
    mac->cell_cfg(sched_cfg);

    // Re-initialize Paging Manager tracking references
    pending_paging.reset(new paging_manager(cfg.sibs[1].sib2().rr_cfg_common.pcch_cfg.default_paging_cycle.to_number(),
                                            cfg.sibs[1].sib2().rr_cfg_common.pcch_cfg.nb.to_number(),
                                            cfg.etws_present,
                                            cfg.cmas_present));
    if (pending_paging) {
      pending_paging->trigger_system_info_modification();
    }
    // ========================================================================
  }
  // ==========================================================================
