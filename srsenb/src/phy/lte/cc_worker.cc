...
cc_worker::~cc_worker()
{
  ...
  // Free both UL instances
  for (int i = 0; i < 2; i++) {
    srsran_enb_ul_free(&enb_ul[i]);
  }
...
void cc_worker::init(phy_common* phy_, uint32_t cc_idx_)
{
  ...
  uint32_t         nof_ant    = phy->get_nof_ports(cc_idx);

  // 1. Initialize cell and allocate buffers for all available ports
  for (uint32_t p = 0; p < nof_ant; p++) {
    signal_buffer_rx[p] = srsran_vec_cf_malloc(2 * sf_len);
    if (!signal_buffer_rx[p]) {
      ERROR("Error allocating RX memory for port %d", p);
      return;
    }
    ...
    if (!signal_buffer_tx[p]) {
      ERROR("Error allocating TX memory for port %d", p);
      return;
    }
    srsran_vec_cf_zero(signal_buffer_tx[p], 2 * sf_len);
  }

  // 2. Initialize Downlink
  ...
  // 3. === UPDATED UL INITIALIZATION FOR DF ===
  // Initialize multiple UL instances (one per physical antenna)
  // We cap this at SRSRAN_MAX_PORTS or your array size (usually 2 for DF)
  for (uint32_t i = 0; i < nof_ant && i < 2; i++) {
    // Pass 'i' as the antenna index to the library
    if (srsran_enb_ul_init(&enb_ul[i], signal_buffer_rx[i], nof_prb, i)) {
      ERROR("Error initiating ENB UL for antenna %d", i);
      return;
    }

    if (srsran_enb_ul_set_cell(&enb_ul[i], cell, &phy->dmrs_pusch_cfg, nullptr)) {
      ERROR("Error setting cell for ENB UL antenna %d", i);
      return;
    }

    // Support for 8-bit decoder if enabled
    if (phy->params.pusch_8bit_decoder) {
      enb_ul[i].pusch.llr_is_8bit        = true;
      enb_ul[i].pusch.ul_sch.llr_is_8bit = true;
    }
  }

  // 4. Setup RNTIs in PHY
  ...
  Info("Component Carrier Worker %d configured with %d antennas and %d PRB", cc_idx, nof_ant, nof_prb);
...
void cc_worker::work_ul(const srsran_ul_sf_cfg_t& ul_sf_cfg, stack_interface_phy_lte::ul_sched_t& ul_grants)
{
  ...
  // Process UL signal
  srsran_enb_ul_fft(&enb_ul[0]);
  srsran_enb_ul_fft(&enb_ul[1]);
                                  ...
                                  srsran_pusch_res_t&                        pusch_res)
{
  ...
  if (srsran_ra_ul_dci_to_grant(&enb_ul[0].cell, &ul_sf, &ul_cfg.hopping, &ul_grant.dci, &grant)) {
  ...
  if (pusch_res.data) {
    // 1. PRIMARY DECODE: Use Antenna 0 for the actual data to the stack
    enb_ul[0].chest_res.antenna_idx = 0;
    if (srsran_enb_ul_get_pusch(&enb_ul[0], &ul_sf, &ul_cfg.pusch, &pusch_res)) {
      Error("Decoding PUSCH Ant 0 for RNTI %x", rnti);
      return false;
    }

    // 2. DF ONLY: Run Antenna 1 ONLY for the dashboard estimates
    // We create a dummy result struct so Ant 1 doesn't overwrite Ant 0's data
    srsran_pusch_res_t dummy_res = {};
    enb_ul[1].chest_res.antenna_idx = 1;
    srsran_enb_ul_get_pusch(&enb_ul[1], &ul_sf, &ul_cfg.pusch, &dummy_res);
  }
  ...
  float snr_db = enb_ul[0].chest_res.snr_db;
  ...
  if (snr_db >= PUSCH_RL_SNR_DB_TH) {
    ...
    if (ul_cfg.pusch.meas_ta_en and not std::isnan(enb_ul[0].chest_res.ta_us) and not std::isinf(enb_ul[0].chest_res.ta_us)) {
      phy->stack->ta_info(ul_sf.tti, rnti, enb_ul[0].chest_res.ta_us);
    }
  }
  ...
  if (ul_grant.data != nullptr) {
    ...
    ue_db[rnti]->metrics_ul(ul_grant.dci.tb.mcs_idx,
                            enb_ul[0].chest_res.epre_dBfs - phy->params.rx_gain_offset,
                            enb_ul[0].chest_res.snr_db,
  ...
  for (uint32_t i = 0; i < nof_pusch; i++) {
    ...
    if (ul_grant.data != nullptr) {
      ...
      if (logger.info.enabled()) {
        char str[512];
        srsran_pusch_rx_info(&ul_cfg.pusch, &pusch_res, &enb_ul[0].chest_res, str, sizeof(str));
...
int cc_worker::decode_pucch()
{
  ...
  for (auto& iter : ue_db) {
    ...
    if (SRSRAN_RNTI_ISUSER(rnti) and phy->ue_db.is_pcell(rnti, cc_idx)) {
      ...
      if (ret > SRSRAN_SUCCESS) {
        // Decode PUCCH
        if (srsran_enb_ul_get_pucch(&enb_ul[0], &ul_sf, &ul_cfg.pucch, &pucch_res)) {
...
int cc_worker::read_ce_abs(float* ce_abs)
{
  ...
  srsran_vec_abs_dB_cf(enb_ul[0].chest_res.ce, -80.0f, &ce_abs[g], SRSRAN_NRE * phy->get_nof_prb(cc_idx));
  return sz;
}
...
int cc_worker::read_ce_arg(float* ce_arg)
{
  ...
  int g = (sz - SRSRAN_NRE * phy->get_nof_prb(cc_idx)) / 2;
  srsran_vec_arg_deg_cf(enb_ul[0].chest_res.ce, -80.0f, &ce_arg[g], SRSRAN_NRE * phy->get_nof_prb(cc_idx));
  return sz;
}

int cc_worker::read_pusch_d(cf_t* pdsch_d)
{
  int nof_re = enb_ul[0].pusch.max_re;
  memcpy(pdsch_d, enb_ul[0].pusch.d, nof_re * sizeof(cf_t));
  return nof_re;
}
...
int cc_worker::read_pucch_d(cf_t* pdsch_d)
{
  ...
  memcpy(pdsch_d, enb_ul[0].pucch.z_tmp, nof_re * sizeof(cf_t));
  return nof_re;
}
