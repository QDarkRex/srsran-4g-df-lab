...
public:
    ...
    logger(srslog::fetch_basic_logger("RRC"))
  {
    for (subframe_info& sf_obj : sf_pending_pcch) {
      ...
      for (pcch_info& pcch : sf_obj.pending_paging) {
        ...
        pcch.pcch_msg.msg.c1().paging().sys_info_mod_present           = true; // systemInfoModification = true
  ...
  void trigger_system_info_modification()
  {
    for (subframe_info& sf_obj : sf_pending_pcch) {
      std::lock_guard<std::mutex> lock(sf_obj.mutex);
      for (size_t sfn_idx = 0; sfn_idx < T; ++sfn_idx) {
        pcch_info& pcch = sf_obj.pending_paging[sfn_idx];
        if (pcch.is_tx()) {
          pcch.clear();
        }
        if (pcch.pdu == nullptr) {
          pcch.pdu = srsran::make_byte_buffer();
        }
        if (pcch.pdu != nullptr) {
          pcch.pdu->clear();
          pcch.pcch_msg.msg.c1().paging().sys_info_mod_present = true;
          if (pcch.pcch_msg.msg.c1().paging().paging_record_list.size() == 0) {
            pcch.pcch_msg.msg.c1().paging().paging_record_list_present = false;
          } else {
            pcch.pcch_msg.msg.c1().paging().paging_record_list_present = true;
          }
          asn1::bit_ref bref(pcch.pdu->msg, pcch.pdu->get_tailroom());
          if (pcch.pcch_msg.pack(bref) != asn1::SRSASN_ERROR_ENCODE_FAIL) {
            pcch.pdu->N_bytes = (uint32_t)bref.distance_bytes();
          } else {
            pcch.pdu.reset();
          }
        }
